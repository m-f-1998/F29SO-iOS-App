...
class HireBikeController: FormViewController, PKPaymentAuthorizationViewControllerDelegate {
    ...
    override func viewDidLoad() {  // Do any additional setup after loading the view, typically from a nib.
        ...
        <<< ButtonRow(){
            ...
            if !(formvalues["accessories"] as! Set<String>).isEmpty {
    ...
    func paymentAuthorizationViewController(_ controller: PKPaymentAuthorizationViewController, didAuthorizePayment payment: PKPayment, handler completion: @escaping (PKPaymentAuthorizationResult) -> Void) { // Process Booking In Database
        let formvalues = self.form.values()
        ...
        do {
            ...
            sendRequest(bookingsURL, method: .post, parameters: ["email": UserDefaults.standard.string(forKey: "userDetails")!, "password": keychainPassword!, "bookStart": formvalues["bookStart"]!!, "bookEnd": formvalues["bookEnd"]!!], completionHandler: { (data, response, error) in
                guard let data = data else { return }
                do {
                    let json = try JSONSerialization.jsonObject(with: data, options: .allowFragments) as! [String : Any]
                    let returnMessage = json["message"] as! NSString
                    self.alert(title: String(returnMessage), message: "", style: .alert)
